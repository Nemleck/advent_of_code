...
def get_type(cards_set: str):
    ...
    if "J" in cards_set and game_type != "five of a kind":
        best = 0
        best_char = ""
        for key in chars.keys():
            if chars[key] > best and key != "J":
                best = chars[key]
                best_char = key

        # print(f"Best char is {best_char}, with {best} occurences.")
        game_type = get_type(cards_set.replace("J", best_char))
